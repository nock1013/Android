...
public class MainActivity extends AppCompatActivity {
    ...
    /*
            ...
            오랫동안 처리해야 하는 작업을 UI쓰레드()에 정의하면 안된다.
                                ---------------------------------
                                 ^
                                 |__별도의 작업 쓰레드를 정의하고 실행

     */
    public void btnNoThread(View view){
        for (progressVal=1;progressVal<100;progressVal++){
            progressBar.setProgress(progressVal);
            SystemClock.sleep(1000); //1초동안 쉬게 만드는 작업(1초동안 멈춰있는 효과)
        }
    }
    //개발자가 만든 쓰레드 안에서 UI를 변경
    //- 잠정적은 문제점을 갖고 있는 방법(UI의 변경은 UI쓰레드에서만 작업)
    public void useThread(View view){
        //프로그래스바의 진행상태가 출력되도록 설정
    ...
    public void userHandler(View view){
        new Thread(new Runnable() {
            ...
            public void run() {
                for (progressVal=1;progressVal<=100;progressVal++){
                    //handler가 갖고 있는 Message객체를 매개변수로 전달하며 작업을 의로
