...
public class MainActivity extends AppCompatActivity {
    ...
    public void setFragment(String name){
        //fragment객체를 관리하는 관리자 객체를 구한다.getSupportFragmentManager();
        FragmentManager fragmentManager = getSupportFragmentManager();
        //fragment작업을 시작하기 위한 트랜잭션 객체를 구한다.
        FragmentTransaction transaction = fragmentManager.beginTransaction();
        switch (name){
            case "first" :
                //지정한  fragment로 특정영역을 교체하는 작업 => transaction.replace()
        ...
        //commit은 스케줄 고려해서 적당한 시기에 변경해달라고 요청을 의로
        //commitNow보다 commit을 더 선호
        transaction.commit();
